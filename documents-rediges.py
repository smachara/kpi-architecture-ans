...
colors_bar = ['b', 'r', 'g', 'c', 'm', 'y', 'k']  # Colores para las barras de documentos rédigés
colors_line = ['navy', 'darkred', 'darkgreen', 'darkcyan', 'purple', 'goldenrod', 'black']  # Colores para las líneas del % avancement
...
for i, projet in enumerate(data['Projet'].unique()):
    ...
    # Dibujar las barras de documentos rédigés
    ax1.bar(index + i * bar_width, subset['Documents rédigés'], bar_width, label=f'Docs rédigés {projet}', color=colors_bar[i % len(colors_bar)])
    ax1.bar(index + i * bar_width, subset['Documents en rédaction'], bar_width, bottom=subset['Documents rédigés'], label=f'Docs en rédaction {projet}', color=colors_bar[(i + 1) % len(colors_bar)])
...
for i, projet in enumerate(data['Projet'].unique()):
    subset = data[data['Projet'] == projet]
    ax2.plot(index + i * bar_width + bar_width / 2, subset['% Avancement rédaction'], 'o-', color=colors_line[i % len(colors_line)], label=f'% Avancement rédaction {projet}')
